"""Streamlit UI for an HSK vocabulary quiz.
...
def render_summary() -> None:
    ...
    total = st.session_state.get("num_questions", len(st.session_state["questions"]))
...
def main() -> None:
    st.set_page_config(page_title="Quiz vocabulaire HSK", page_icon="📘", layout="centered")
    ensure_seeded()

    st.title("Quiz vocabulaire HSK")
    st.write(
        "Testez vos connaissances en vocabulaire du HSK. "
    ...
    if st.sidebar.button("Lancer un nouveau quiz"):
        ...
        reset_quiz(selected_quiz_key, num_questions, seed=seed)
        _trigger_rerun()
        return
